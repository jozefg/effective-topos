...
\begin{defn}\label{defn:complete:completefibration}\hfill
  ...
  \item $P : C \to \Etop$ is complete if $C$ has fiberwise finite
    limits and a right adjoint to reindexing. Moreover, both of these
    are preserved by reindexing.
  \item $P : C \to \Etop$ is weakly complete if $C$ if given a finite
    ...
    reindexing preserves all finite limits and indexed products when
    they exist.
  \end{itemize}
\end{defn}
...
\begin{proof}
  ...
  \[
    \prod_{\gamma^*(\alpha \circ \beta)} ((\alpha \circ \beta)^*\gamma)^*(B)
  \]
  By Beck-Chevalley we have that the above is isomorphic to
  \[
    \gamma^*(\prod_{\alpha \circ \beta} B)
  \]
