...
\begin{proof}
  ...
  \[
    \begin{tikzcd}
      J \ar[d, swap, twoheadrightarrow, "\alpha"] \ar[rd, "f_0"] \ar[rr, "f_1"] && J \ar[d, twoheadrightarrow, "\alpha"]\\
  ...
  \[
    \begin{tikzcd}
      J \ar[d, swap, twoheadrightarrow, "\alpha"] \ar[rr, "f_1"] && J \ar[d, twoheadrightarrow, "\alpha"]\\
      I \ar[dd, swap, "A"] \ar[rr, "g_1"] \ar[dr, "g_0"]  & & I \ar[dd, "B"]\\
      & C_1 \ar[dl, swap, "\partial_0"] \ar[dr, "\partial_1"] &\\
      C_0 && C_0
    \end{tikzcd}
  \]
  ...
  \]
  However, since $\iota$ is a natural isomorphism, we must have then
  that $p^*(f_1) = q^*(f_1)$ on the nose. Finally, since $q$ and $p$
  are determined by the pullback of an epi, they themselves are
  epis. Therefore, since we have that the following diagrams commute,
  we may conclude that $f_1^*(q) = f_1^*(p)$.
  \[
    \begin{tikzcd}
      J \times_J (J \times_J J) \ar[r] \ar[d, swap, "p^*(f_1)"]& J \times_J J \ar[d, "p"]\\
      J \ar[r, swap, "f_1"] & J
    \end{tikzcd}
    \qquad
    \begin{tikzcd}
      J \times_J (J \times_J J) \ar[r] \ar[d, swap, "q^*(f_1)"]& J \times_J J \ar[d, "q"]\\
      J \ar[r, swap, "f_1"] & J
    \end{tikzcd}
  \]
  This tells us that in the following diagrams, $h_p = h_q$.
  \[
    \begin{tikzcd}
      J \times_J J \times_J J \ar[dr, "h_p", dashed] \ar[ddr, bend right, "p"] \ar[rrd, bend left, "f_1 \times f_1"]&&\\
      & J \times_J (J \times_J J) \ar[r] \ar[d, swap, "p^*(f_1)"]& J \times_J J \ar[d, "p"]\\
      & J \ar[r, swap, "f_1"] & J
    \end{tikzcd}
    \hspace{-3em}
    \begin{tikzcd}
      J \times_J J \times_J J \ar[dr, "h_q", dashed] \ar[ddr, bend right, "q"] \ar[rrd, bend left, "f_1 \times f_1"]&&\\
      & J \times_J (J \times_J J) \ar[r] \ar[d, swap, "q^*(f_1)"]& J \times_J J \ar[d, "q"]\\
      & J \ar[r, swap, "f_1"] & J
    \end{tikzcd}
  \]
  Therefore, we have that $f_1 \circ p = f_1 \circ q$ and we have our
  desired extension $g_1 : I \to J$ so that $g_1 \circ \alpha =
  f_1$. We an $f_0$ which satisfies similar properties. We essentially
  repeat the procedure we have just done because by assumption we have
  that
  \[
    m \circ \langle f_0 \circ f_1^*(p), (\iota_B)_0 \circ p^*(f_1) \rangle =
    m \circ \langle (\iota_A)_0, f_0 \circ f_1^*(q) \circ (\iota_A)_1 \rangle
  \]
  Since $(\iota_{-})_0$ is an internal natural isomorphism, we can
  conclude that
  \begin{align*}
    m \circ \langle f_0 \circ f_1^*(p), (\iota_B)_0 \circ p^*(f_1) \rangle &=
    m \circ \langle (\iota_A)_0, f_0 \circ f_1^*(q) \circ (\iota_A)_1 \rangle\\
