...
\begin{proof}
  ...
  $Z$ and the domain of $A^*(q)$ as $W$. In order to show this is
  reflexive, we want to show that $\Delta : X \to X \times X$ factors
  through $R = \langle A^*(p), A^*(q) \circ \alpha \rangle$. In order
  to show that this holds, let us show that $\Delta^*(R)$ has a global
  point (an obvious equivalence). However,
  \[
    \Delta^*(R) = \langle \Delta^*(A^*(p)), \Delta^*(\alpha) \circ \Delta^*(A^*(q)) \rangle
  \]
  We know by the unit condition (the first coherence condition on
  $\alpha$) that $\Delta^*(\alpha) = 1$ so this is equivalent to
  \[
    \langle \Delta^*(A^*(p)), \Delta^*(A^*(q)) \rangle
  \]
  However, $(p, q)$ is a kernel pair and therefore it has a global
  point which is preserved by $\Delta^* \circ A^*$ so we're done.

  To show that it is symmetric we first observe that $p, q$ is
  symmetric as a kernel pair. Therefore, we have that
  $\twist \circ (p \times q) \cong p \times q$ in the slice above
  $I \times I$. Therefore
  $A^*(\twist) \circ A^*(p) \times A^*(q) \cong A^*(p) \times A^*(q)$
  and since $A^*(\twist) = \twist$ (pullback preserves products) we
  then have that $A^*(q), A^*(p)$ is also symmetric and since $\alpha$
  is an iso and so so is $A^*(p), A^*(q) \circ \alpha$ as
  required. Transitivity is the most involved. For this start by
  forming the pullback
  \[
    \begin{tikzcd}
      Z \times_X Z \ar[d, swap, "a"] \ar[r, "b"] & Z \ar[d, "A^*(q) \circ \alpha"]\\
      Z \ar[r, swap, "A^*(p)"] & X
    \end{tikzcd}
  \]
  Then we want to show that there exists an arrow $h$ below, letting
  $R' = \langle A^*(p) \circ b, A^*(q) \circ \alpha \circ a \rangle$
  \[
    \begin{tikzcd}
      Z \times_X Z \ar[dr, swap, "R'"]
      \ar[rr, dashed, "h"]&&
      Z \ar[dl, "R"]\\
      & X \times X &
    \end{tikzcd}
