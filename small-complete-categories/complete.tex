...
satisfies the axiom of choice but since the effective topos does not,
we are then left with two different notions of finite limits. The one
based on the existence of adjoints essentially asserts the global
existence of a limit cone and will be called the \emph{strong} notion
of finite limits. The one based on the internal language is implied by
this and is called the \emph{weak} or \emph{local} version of a finite
limit.
...
our notion of weak completeness to \emph{indexed} diagrams and that
the limits merely need to exist fiberwise and internally\footnote{The
  crucial move here is not really considering slices, it is the
  ability to have to show this limit exists fiberwise! This statement
  is strictly weaker than the previous notion of weak completeness
  without the axiom of choice. We do not need a pseudo-inverse to move
  the diagram backwards, we can use a different construction to
  construct an \emph{internally} isomorphic diagram.}. An indexed
...
\begin{thm}\label{thm:complete:preservation}
  ...
  \item Weak equivalences preserve and reflect weak completeness of
    internal categories.
  \end{enumerate}
\end{thm}
\begin{proof}
  ...
  being complete. Suppose we have a diagram $d : D \to I^*(C)$ in
  $\Etop/I$. We can trivially extend this to a diagram
  $I^*(F) \circ d : D \to I^*(C')$. By assumption, there is an
  internal limit of this diagram. Let us denote the formula for a
  limit existing as $\phi$, eg
  \begin{align*}
    \forall i : I.\ \exists l : C_0'.\ \exists f : C_1'^{D_0^i}.\ &(\forall d : D_0.\ f : l \to (I^*(F) \circ d)(d)) \land\\
    &(\forall g : D_1^i.\ f(\partial_1(g)) = (I^*(F) \circ d)(g)) \circ f(\partial_0(g)) \land\\
    &\text{`This cone is universal'}
  \end{align*}
  Here we have used $X^i$ to signify the $i$th fiber of some object
  $f : X \to I$, defined in the standard way as
  $\{x : X \mid f(x) = i\}$.

  Since $I^*$ (indeed pullback functors in general) are logical in a
  topos, we know that $I^*(F)$ is full, faithful and internally surjective
  internally. Therefore, we know that the internal formulas
  \begin{gather*}
    \forall x : C_0'.\ \exists y : C_0.\ F_0(y) = x\\
    \forall f : C_0'.\ \exists g : C_1.\ F_1(g) = f\\
    \forall f, g : C_1.\ f = g \iff F_1(f) = F_1(g)
  \end{gather*}
  Using these formulas along with the statement of an internal limit
  given above, simple manipulation is sufficient to conclude the
  validity of the following formula.
  \begin{align*}
    \forall i : I.\ \exists l : C_0.\ \exists f : C_1^{D_0}.\ &(\forall d : D_0^i.\ f : l \to (I^*(F) \circ d)(d)) \land\\
    &(\forall g : D_1^i.\ f(\partial_1(g)) = (I^*(F) \circ d)(g)) \circ f(\partial_0(g)) \land\\
    &\text{`This cone is universal'}
  \end{align*}
  Which is precisely the desired internal limit for $I^*(C)$.

  For preservation, suppose that $F : C \simeq C'$ and that $C$ is
  complete. We need to show that $C'$ is as well. Suppose that there
  is a functor $d : D \to I^*(C')$, the central challenge is that we
  cannot immediately factor $d$ through $F$.


  For this, suppose that we have a diagram $d : D \to I^*(C')$. By
  taking the iso-comma square formed by $d$ and $I^*(F)$ with the top
  corner $D'$, we end up with a diagram on $I^*(C)$,
  $d' : D' \to I^*(C)$. The limit of this diagram is must exist
  internally by assumption and it is trivial to check that it is
  mapped to a in $I^*(C')$ by $I^*(F)$. Finally, considered fiberwise,
  we have that the diagram $I^*(F) \circ d' \cong d$ internally. In
  particular, this means that the limit we have just constructed is
  the desired one.
\end{proof}
...
\begin{defn}\label{defn:complete:completefibration}\hfill
  \begin{itemize}
  \item $P : C \to \Etop$ is complete if $P$ has fiberwise finite
    limits and a right adjoint to reindexing. Moreover, limits are
    preserved by reindexing and indexed products satisfy
    Beck-Chevalley.
  \item $P : C \to \Etop$ is weakly complete if $P$ if there is a full
    subcategory of $s(\Etop) \included \Etop$ so that $P$ is complete
    as a fibration over $s(\Etop)$ and every object in $P$ is covered
    by an object in $s(\Etop)$.
  \end{itemize}
\end{defn}
This definition of weakly complete is directly transcribed from
\citet{Hyland:88} as opposed to the definition from
\citet{Hyland:90}. The latter definition being essentially an
unfolding of the former, but precisely specifying the Beck-Chevalley
condition and reindexing compatibility is quite cumbersome.

An important property of indexed products mentioned above is the so
...
\begin{proof}
  ...
  cartesian). But now we're stuck, there is now reason why $\gamma^*$
  must be full because $\gamma$ is epi, not split epi and nothing has
  been assumed allowing us to move from $Q^L$ to $Q^K$.
\end{proof}
