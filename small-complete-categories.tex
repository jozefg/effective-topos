...
\begin{lem}\label{lem:orth:orthsubobjects}
  If $A \epi 1$ and $A$ is internally projective then $\orth(A)$ is
...
\begin{proof}
  ...
  \[
    \begin{tikzcd}
      f' \ar[d, rightarrowtail] \ar[r, rightarrowtail] & {f'^{I^*(A)}} \ar[d, rightarrowtail, "m_2"]\\
      f \ar[r, rightarrowtail, swap, "m_1"] & {f^{I^*(A)}}
    \end{tikzcd}
  \]
  Now since the bottom arrow is an iso, so too is the top and we're
  done. In order to show this is a pullback
\end{proof}
